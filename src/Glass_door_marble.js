...
export default class GlassWindow extends Component {
  ...
  step = (ctx) => {
    ...
    this.agents.forEach(agent => {
      ...
      const noise = simplex.noise2D(x / 100, y / 100);
      const dAngle = PI / 2 * (3 + (noise - noiseMin) / noiseRange);

      const angle1 = 0.7 * angle + 0.3 * PI / 2 * (3 + (noise - noiseMin) / noiseRange);
      ...
      if (random() < 0.01) {
        ...
        const dAngle = (random() - 0.5) * 2 * PI;
        agent.angle = angle1 - dAngle / 4;
        branch.angle = angle1 + dAngle * 3 / 4;
